...
def bigramLaplace(pathToTrainLang1, pathToTrainLang2, pathToTrainLang3, pathToTrainLang4, pathToTrainLang5, pathToTrainLang6
    , pathToEvaluateSpanglish, uniBackoff= .5, biBackoff= .5):
    ...
    #en_dev1 = open(pathToEvaluateSpanglish, "r", encoding="utf-8")
    ...
    #parse twitter csv and update dev set bigram and unigram counts 
    with open(pathToEvaluateSpanglish, 'r') as f:  #opens data file
        reader = csv.reader(f)
        for row in reader:
            ...
            for token in row:         
                ...
                #    ##adding to temporary sentence which will be parsed into bigrams
                #    #making it lower case as a means of preprocessing. words of different case but same spelling are the same type for my purposes
                tempSentence1.append(token.lower())
                tempSentence2.append(token.lower())
                tempSentence3.append(token.lower())
                tempSentence4.append(token.lower())
                tempSentence5.append(token.lower())
                tempSentence6.append(token.lower())
            ...
            #Find top two of these log likelihoods 
            #If top two highest log linear evaluated probabilities are spanish and english in any order, evaluate as Spanglish tag.
            #Otherwise evaluate as other tag.
            
            #gets key of number one language
            LangOne = max(probDict, key=probDict.get)
            
            #removes number one language so I can maximize and find number 2 language
            del probDict[LangOne]
            
            #gets key of number two language
            LangTwo = max(probDict, key=probDict.get)

            #note that for our input command lang2 is english and lang4 is spanish. 
            #you must run our program with those training inputs as the 2nd and 4th training language in order for this program to work
            if((LangOne=='Lang2' or LangOne=='Lang4' )and (LangTwo=='Lang2' or LangTwo=='Lang4')):
                predictedLang.append('Spanglish')
            else:
                predictedLang.append('Other')
            
            print("\nFor the following sentence:")
            print(row)
            print("Predicted Language One is %s while Language Two is %s" %(LangOne, LangTwo))
            print("Note that 1 is French, 2 is English, 3 is German, 4 is Spanish, 5 is Italian, 6 is Dutch")
                
            #append the actual language this dev set is. We are denotating the spanish and english multiclass as spanglish
            actualLang.append('Spanglish')
    ...
    #for debugging
    print()
    for i in range(0,len(predictedLang)):
    ...
    print("length of the two sets is")
    print(len(predictedLang))
    print(len(actualLang))
    print("\n")
    
    print("Now calculating precision recall and f1 scores\n")
    #I am no longer using scikit learn because it does not work well with only two labels and one label having no true cases
    #It would be better to create a set of tweets that we know to not be spanglish as well as a set of spanglish tweets but our immediate goal
    #was to accurately identify spanglish tweets and in the interest of that looking at only spanglish tweets was helpful. However, our dataset
    #was not very useful. The obviously spanglish tweets were picked up easily. The ones that were ambiguous with weird spellings and typos
    #or only one english word, were easily predicted for the correct primary language but did not meet criteria for the second english language. 
    
    #precision is true positives / true positives + false positives. Every case of other is a false positive
    precision = (countRight/ (countRight + countWrong))
    
    #recall is true positives / true positives + false negatives. Every case of other is also a false negative since we only have one label.
    recall = (countRight / (countRight + countWrong) ) 
    
    print("\nTotal number of sentences in dev set 1 is %d"%(countDevLang1))
